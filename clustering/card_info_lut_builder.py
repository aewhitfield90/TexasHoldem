...
# Import the required classes from local modules
from card_combos import CardCombos
from game_utility import GameUtility
from preflop import compute_preflop_lossless_abstraction
...
class CardInfoLutBuilder(CardCombos):
    ...
    def __init__(
        ...
        self.card_info_lut_path = Path(save_dir) / "card_info_lut.joblib"
        self.centroid_path = Path(save_dir) / "centroids.joblib"
        ...
        try:
            ...
            log.info("No existing data found. Starting new clustering process.")
    ...
    def simulate_get_ehs(self, game: GameUtility) -> np.ndarray:
        ...
        for _ in range(self.n_simulations_river):
            opp_hand = game.opp_hand.tolist()  
            idx: int = game.get_winner(opp_hand)
            #idx = game.get_winner()
            # increment the win rate for winner/tie  
    ...
    def create_card_lookup(clusters: np.ndarray, card_combos: np.ndarray) -> Dict:
        ...
        # Logging the lengths of the inputs to ensure they are correctly aligned
        log.debug(f"Number of clusters: {len(clusters)}")
        log.debug(f"Number of card combinations: {len(card_combos)}")

        if len(clusters) != len(card_combos):
            log.error("Mismatch in the number of clusters and card combinations.")
            raise ValueError("The number of clusters must match the number of card combinations.")
